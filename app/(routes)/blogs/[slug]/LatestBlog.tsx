  ...
  return (
    <div className="grid grid-cols-1 gap-6 px-4 md:grid-cols-2 lg:grid-cols-3">
      {latestPosts.map((post) => (
        <div
          key={post.slug}
          className="sizeSubtitle group cursor-pointer overflow-hidden rounded-xl border border-borderPrimary bg-white shadow-sm transition-all hover:shadow-md dark:border-borderPrimary dark:bg-bgDarkSecondary"
        >
          <Image
            src={post.coverImage}
            alt={post.title}
            ...
            loading="lazy"
          />
          <Link href={`/blogs/${post.slug}`} className="flex flex-col p-4" scroll={true}>
