...
import { PiGithubLogo, PiInstagramLogo, PiLinkedinLogo, PiList, PiX } from "react-icons/pi"
...
export const HeaderWrapper = () => {
  ...
  return (
    <header className="fixed top-0 z-50 w-full bg-white/80 backdrop-blur-sm lg:static">
      <div className="grid grid-cols-2 items-center border-b border-borderPrimary px-5 py-3 text-[14px] text-textPrimary lg:grid-cols-[200px_1fr_200px] lg:px-3">
        ...
        <div className="flex justify-end lg:hidden">
          <button className="flex items-center gap-2 rounded-full border p-2 text-2xl" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            {isMenuOpen ? <PiX className="text-2xl" /> : <PiList className="text-2xl" />}
          </button>
        </div>
