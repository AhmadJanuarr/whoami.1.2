          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {PROJECTS.slice(0, 3).map((project) => (
              <FollowerPointerCard key={project.id} title={<TitleComponent title={project.author} avatar={project.authorAvatar} />}>
                <div className="group relative h-full overflow-hidden rounded-2xl border border-zinc-100 bg-white p-4 transition duration-300 dark:bg-bgDarkSecondary">
                  ...
                  <div className="sizeSubtitle mt-6 flex flex-col gap-4 p-2">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-normal text-textSecondary dark:text-textDarkSecondary">{project.date}</span>
                    </div>
                    ...
                    <p className="line-clamp-3 font-normal text-textSecondary dark:text-textDarkSecondary">{project.description}</p>
                    <Link
                      href={project.href}
                      className="sizeSubtitle rounded-xl border bg-zinc-100 px-4 py-2 text-center text-sm text-textSecondary transition-all duration-200 hover:font-semibold hover:text-textPrimary dark:bg-bgDarkSecondary dark:text-textDarkSecondary"
                    >
                      View Project
                    </Link>
                  </div>
